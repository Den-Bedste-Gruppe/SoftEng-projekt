...
Scenario: The worker registers some hours spent on an activity
	...
	And the activity has a total of 0 hours spent
...
Scenario: The worker registers hours spent on a non-project activity
	Given that there is a non-project activity
	When the worker registers 5 hours on the activity
	Then the error message "Can't register hours on a non-project activity" is given
